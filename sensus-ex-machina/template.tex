...
\begin{abstract}
  Performance benchmarks measure what machines can do, but not how it feels to be with them. This paper introduces \emph{sensus ex machina}, \emph{feeling from the machine}, as the phenomenological threshold where humans experience artificial intelligence as presence rather than tool. Drawing on embodied cognition research, phenomenology, and close analysis of the film \emph{Ex Machina}, this framework identifies the architecture of \emph{feltness}: the pre-reflective sense that one is with a subject rather than operating a system. Feltness emerges along multiple axes, such as microexpressions, vulnerability, persistence, embodiment, identity, forming a multidimensional \emph{feltness space} with local maxima and failure modes, including the uncanny valley, forming a topological \emph{feltness landscape}. Each observer-observed pairing traces its own \emph{feltness profile}. This framework argues that the path to felt artificial general intelligence lies in ontological convergence, not in greater cognitive power: designing machines that share humanity's locality, breakability, and irreversibility. Feltness is irreducibly projective, with implications for alignment, machine ethics, and the weaponization of presence at scale.
\end{abstract}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.88\textwidth]{feltness_space.png}}
